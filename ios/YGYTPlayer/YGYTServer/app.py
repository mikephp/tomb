...
class index:
    ...
    def GET(self):
        ...
        query = {}
        if version == 1:
            lang = '' if lang_idx > len(
                LANG_MAPPING_V1) else LANG_MAPPING_V1[lang_idx]
            if lang:
                query['lang'] = lang
        elif version == 2:
            lang = '' if lang_idx > len(
                LANG_MAPPING_V2) else LANG_MAPPING_V2[lang_idx]
            if lang == 'en':
                query['lang'] = {'$in': ['en', 'fr', 'de', 'es', 'it']}
            elif lang:
                query['lang'] = lang
        # print('query on mongo = %s' % query)
        projection_list = [
            'title', 'description', 'viewcount', 'thumb', 'key', '_id', 'bigthumbhd']
        ...
        rs = CF.FACT_TABLE.find(
            query, skip=s, limit=c, projection=projection_dict, sort=sort_pred)
        vds = []
        for r in rs:
            vd = {}
            vd['tt'] = r['title']
...
class popular:
    ...
    def GET(self):
        js = {
            'terms': ['yoga with adriene', 'best sex with yoga', 'free ponography']}
        web.header('Content-Type', 'application/json')
        return json.dumps(js)
