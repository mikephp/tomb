            ...
            for (i,v) in enumerate(item):
                ...
                if key.startswith('Label'):
                    ...
                    def format_key(key):
                        ...
                        elif key == 'Author': return 'author'
                        elif key == 'Playlist': return 'playlist'
                        elif key == 'Language': return 'lang'
                        else: return key
                    key2 = format_key(key)
                    record[key2] = v
            if 'playlist' in record:
                ...
                for q in qs:
                    if q.startswith('v') and not 'cover-id' in record:
                        ...
                        record['playlist'] = 'https://www.youtube.com/playlist?%s' % (q)
...
def nc_video_info(video_id):
    ...
    return data
def video_info(video_id): return mongo_cache_func(CF.VIDEO_TABLE, video_id, nc_video_info, video_id)
...
def nc_playlist_info(playlist_id):
    ...
    return data
def playlist_info(playlist_id): return mongo_cache_func(CF.VIDEO_TABLE, playlist_id, nc_playlist_info, playlist_id)
