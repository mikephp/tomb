...
def cache_index(country, genre):
    cache_file = 'cache/itunes-%s-%d' % (country, genre)
    return cache_file
...
def down_index(comb):
    ...
    for (country, genre) in comb:
        cache_file = cache_index(country, genre)
        if os.path.exists(cache_file):
            continue

        def f(country, genre):
            print('HANDLE. (%s, %d)' % (country, genre))
            url = URL % (country, genre)
            cache_file = cache_index(country, genre)
            r = requests.get(url)
            if r.status_code != 200:
                print('FAILED. url = %s, code = %d' % (url, r.status_code))
                return
...
def parse_index_1(country, genre):
    ...
    for pd in pds:
        ...
        try:
            ...
            continue
        # print(link, pid)
        r = [u for u in session.query(Podcast.pid).filter_by(pid=pid)]
        if r or pid in pids:
            continue
        pids.add(pid)
        r = Podcast(
...
def do_lookup_1(country, genre):
    session = Session()
    rs = session.query(Podcast).filter_by(
    ...
    def f(r):
        ...
        if res.status_code != 200:
            print("FAILED. url = %s, code = %d" % (url, res.status_code))
            return
        # print(res.encoding)
        json = res.json()
...
def do_lookup(comb):
    for (country, genre) in comb:
        do_lookup_1(country, genre)

if __name__ == '__main__':
    comb = make_combination()
    # print('comb size = %d' % (len(comb)))
    # down_index(comb)
    # parse_index(comb)
    do_lookup(comb)
