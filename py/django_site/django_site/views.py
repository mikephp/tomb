    ...
    if profile:
        ...
        if profile.provider in ('facebook', 'google-oauth2',):
            auth = UserSocialAuth.objects.get(
                provider=profile.provider, user_id=user.id)
            ctx['auth_data'] = auth.extra_data
    return render_to_response('django_site/home.html', ctx)
...
def register(request):
    ...
    if request.method == "GET":
        return render_to_response('django_site/register.html', ctx)
...
def dump(request):
    ...
    return render_to_response('django_site/dump.html', ctx)
